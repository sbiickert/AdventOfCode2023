...
//            grid.draw(markers: debugMarkers)
	...
	func solvePartTwo(in grid: AoCGrid2D, at start: AoCCoord2D) -> Int {
		...
		let even_corners = distanceGrid.values.map({$0 as! Int}).filter({$0 % 2 == 0 && $0 > 65}).count
		let odd_corners = distanceGrid.values.map({$0 as! Int}).filter({$0 % 2 == 1 && $0 > 65}).count

		let even_full = distanceGrid.values.map({$0 as! Int}).filter({$0 % 2 == 0}).count
		let odd_full = distanceGrid.values.map({$0 as! Int}).filter({$0 % 2 == 1}).count

		let ext = grid.extent!
		let n = (26501365 - ext.width / 2) / ext.width
		assert(n == 202300)

		let answer = ((n+1)*(n+1)) * odd_full + (n*n) * even_full - (n+1) * odd_corners + n * even_corners;
